...
SAVE_REMINDER PROC
    ...
    mov edx, bufferIndex
    imul edx, 100          ;edx has current offset index (where to write from)
    add eax, edx           ;eax now has current offset

    mov ecx, SIZEOF buffer
    sub ecx, edx           ;ecx has bytes to write(total - bytes_written)
    INVOKE WriteFile, ebx, ADDR eax, ecx, ADDR bytesWritten, 0
...
TO_LOCAL PROC bufferPtr:DWORD, lBufferPtr:DWORD, bufferIndexRef:DWORD ;all three parameters are references/offsets
    ...
    ;COPY BYTE VALUES
    mov [lBufferPtr].date, [ebx]
    inc ebx

    mov [lBufferPtr].month, [ebx]
    inc ebx

    mov [lBufferPtr].year, [ebx]
    inc ebx

    mov [lBufferPtr].hour, [ebx]
    inc ebx

    mov [lBufferPtr].minute, [ebx]
    inc ebx

    mov [lBufferPtr].second, [ebx]
    inc ebx

    ;COPY MESSAGE
    mov edi, [lBufferPtr].message
    mov esi, ebx
    mov ecx, 93
    rep movsb
...
IS_GREATER PROC ;compares 2 obj(args) of type typEntry loads 1 into eax if arg1 greater or equal else 0
    ...
    cmp [esi].month, [edi].month
    jge GorE

    cmp [esi].year, [edi].year
    jge GorE

    cmp [esi].hour, [edi].hour
    jge GorE

    cmp [esi].minute, [edi].minute
    jge GorE

    cmp [esi].second, [edi].second
    jge GorE
    jmp lesser
